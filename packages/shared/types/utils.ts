...
export type DeepestPaths<T, Path extends string[] = []> = T extends string
  ? Path
  : {
      [K in keyof T & string]: DeepestPaths<T[K], [...Path, K]>
    }[keyof T & string]

export type ExcludePath<T, Key extends string, Path extends string[] = []> = T extends string
  ? Path
  : {
...
export type PathTo<T, Key extends string> = Exclude<DeepestPaths<T>, ExcludePath<T, Key>> extends [
  ...infer Path,
  infer _
]
  ? Path
