...
export const websocketExecutionTimeMiddleware = function websocketExecutionTimeMiddleware (): WebsocketMiddleware {
  return async function websocketExecutionTimeMiddleware (ctx, next) {
    const storage = ctx.namespace(ARTUS_FRAMEWORK_WEB_EXECUTION_NAMESPACE)
    ...
    // Show time.
    const { input: { params: { app, req, eventName } } } = ctx
    app.logger.info(
